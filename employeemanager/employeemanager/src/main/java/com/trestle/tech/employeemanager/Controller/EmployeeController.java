...
public class EmployeeController {
	...
	public ResponseEntity<List<Employee>> getByFieldEmployees(@PathVariable String field){
		List<Employee> employees=empservice.sortBasedUponSomeField(field);
		return new ResponseEntity<>(employees,HttpStatus.OK);
	}
	@GetMapping("/pagination/{os}/{size}")
	public ResponseEntity<Page<Employee>> paginationEmployees(@PathVariable int os,@PathVariable int size){
		Page<Employee> em=empservice.paginationBasedData(os, size);
		return new ResponseEntity<>(em,HttpStatus.OK);
	}
}
